...
export const POST: APIRoute = async ({ request }) => {
  ...
  const htmlBody = `
    <h2>Nouvelle demande de démonstration — MonSyndicat.com</h2>
  ...
  const textBody = [
    'NOUVELLE DEMANDE DE DÉMONSTRATION — MONSYNDICAT.COM',
    '',
    `Prénom : ${prenom || '—'}`,
    `Nom : ${nom || '—'}`,
  ...
  try {
    await resend.emails.send({
      from: 'MonSyndicat.com <noreply@monsyndicat.com>',
      to: 'info@monsyndicat.com',
      replyTo: email,
      subject: `Demande de démonstration — ${organisation}`,
      html: htmlBody,
      text: textBody,
    });
